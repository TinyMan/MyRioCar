...
void RemoteControl::serverThread() {
	...
	try {
		...
		while ((n = recvfrom(_sock, headerBuff.data(), headerBuff.size(), 0,
				NULL,
				NULL)) > 0) {
			...
			if (recvfrom(_sock, buff.data(), message.header.length, 0, NULL,
					NULL) > 0) {
...
void RemoteControl::dispatchMessage(message_t message) {
	...
	case MESSAGE_CMD: {
		...
		cout << "CMD Message: speed: " << cmd->speed << ", angle: "
				<< cmd->angle << endl;
